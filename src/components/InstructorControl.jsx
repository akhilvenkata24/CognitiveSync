...
export default function InstructorControl({
  ...
  return (
    <div style={{ padding: '1.5rem', background: '#020308', minHeight: '100vh', display: 'flex', flexDirection: 'column', gap: '1rem', fontFamily: 'var(--font-ui)', color: '#fff' }}>
      ...
      {/* 2. Grid Content */}
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(400px, 1fr))', gap: '1.2rem', flex: 1 }}>
        
        {/* Column 1: Pilot Simulation Sliders & Crisis Scenarios */}
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
          
          {/* Card 1.1: Simulation Sliders */}
          <div className="panel-card" style={{ gap: '1rem' }}>
            <div className="panel-title" style={{ color: '#ff9100', borderBottomColor: 'rgba(255, 145, 0, 0.15)', textShadow: '0 0 8px rgba(255, 145, 0, 0.4)' }}>
              <Sliders size={14} /> Biometric & Gaze Simulation
            </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem', marginTop: '0.4rem' }}>
              <div>
                <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.75rem', color: 'var(--text-secondary)', marginBottom: '0.3rem' }}>
                  <span style={{ fontWeight: 'bold' }}>Attention Score</span>
                  <span style={{ color: '#fff', fontFamily: 'var(--font-hud)' }}>{simState.attentionScore}%</span>
                </div>
                <input 
                ...
                />
                <span style={{ fontSize: '0.55rem', color: 'var(--text-secondary)', textTransform: 'uppercase', display: 'block', marginTop: '0.2rem' }}>
                  Adjusts cockpit state (Focused / Distracted / Emergency Takeover)
                </span>
              </div>

              <div style={{ borderTop: '1px dashed rgba(255,255,255,0.06)', paddingTop: '0.8rem' }}>
                <span style={{ fontSize: '0.7rem', color: 'var(--text-secondary)', display: 'block', textTransform: 'uppercase', marginBottom: '0.6rem', letterSpacing: '0.05em' }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
                  <div>
                    <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.68rem', color: 'var(--text-secondary)', marginBottom: '0.2rem' }}>
                      <span>Gaze Horizontal (X)</span>
                      <span style={{ color: '#fff', fontFamily: 'var(--font-hud)' }}>{simState.gazeX > 0 ? `+${simState.gazeX}` : simState.gazeX}°</span>
                    </div>
                    <input 
                  ...
                  <div>
                    <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.68rem', color: 'var(--text-secondary)', marginBottom: '0.2rem' }}>
                      <span>Gaze Vertical (Y)</span>
                      <span style={{ color: '#fff', fontFamily: 'var(--font-hud)' }}>{simState.gazeY > 0 ? `+${simState.gazeY}` : simState.gazeY}°</span>
                    </div>
                    <input 
                ...
                <span style={{ fontSize: '0.55rem', color: 'var(--text-secondary)', textTransform: 'uppercase', display: 'block', marginTop: '0.4rem' }}>
                  Controls Gaze-Locked Warn Overlay position on pilot screen
                </span>
              </div>
            </div>
          </div>

          {/* Card 1.2: Scenario Injections */}
          <div className="panel-card" style={{ gap: '0.8rem' }}>
            ...
            <span style={{ fontSize: '0.7rem', color: 'var(--text-secondary)', display: 'block' }}>
              Simulate full cockpit crisis sequences:
            </span>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
              <button
                ...
                style={{
                  ...
                  color: 'var(--hud-accent)', padding: '0.7rem 0.9rem', borderRadius: '6px', fontSize: '0.75rem',
              ...
              >
                <div>
                  ...
                  <span style={{ fontSize: '0.6rem', color: 'var(--text-secondary)', fontWeight: 'normal' }}>Oscillates altitude, speed tapes & primary flight HUD</span>
                </div>
                <Compass size={16} />
              </button>
              ...
              <button
                ...
                style={{
                  ...
                  color: 'var(--color-fatigued)', padding: '0.7rem 0.9rem', borderRadius: '6px', fontSize: '0.75rem',
                  cursor: 'pointer', textAlign: 'left', fontWeight: 'bold', transition: 'all 0.2s',
              ...
              >
                <div>
                  ...
                  <span style={{ fontSize: '0.6rem', color: 'var(--text-secondary)', fontWeight: 'normal' }}>Slowly decays operator attention level down to Level 4</span>
                </div>
                <Activity size={16} />
              </button>
              ...
              <button
                ...
                style={{
                  ...
                  color: 'var(--color-distracted)', padding: '0.7rem 0.9rem', borderRadius: '6px', fontSize: '0.75rem',
                  cursor: 'pointer', textAlign: 'left', fontWeight: 'bold', transition: 'all 0.2s',
              ...
              >
                <div>
                  ...
                  <span style={{ fontSize: '0.6rem', color: 'var(--text-secondary)', fontWeight: 'normal' }}>Triggers cascading engine EICAS and generator alarms</span>
                </div>
                <ShieldAlert size={16} />
              </button>
            </div>
        ...
        {/* Column 2: Threat & Alert Controller */}
        <div className="panel-card" style={{ gap: '0.8rem' }}>
          <div className="panel-title" style={{ color: '#ff9100', borderBottomColor: 'rgba(255, 145, 0, 0.15)', textShadow: '0 0 8px rgba(255, 145, 0, 0.4)' }}>
          ...
          <div>
            <span style={{ fontSize: '0.7rem', color: 'var(--text-secondary)', display: 'block', marginBottom: '0.4rem' }}>
              Inject Quick Preset Faults:
            </span>
              ...
              {presets.map((preset, idx) => (
                <button
                  ...
                  style={{
                    ...
                    padding: '0.35rem 0.55rem', borderRadius: '4px', fontSize: '0.62rem', cursor: 'pointer', transition: 'all 0.2s'
                  }}
          ...
          <form onSubmit={handleInjectCustomAlert} style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem', borderTop: '1px dashed rgba(255,255,255,0.08)', paddingTop: '0.6rem' }}>
            <span style={{ fontSize: '0.7rem', color: 'var(--text-secondary)', display: 'block' }}>
            ...
            <input 
              ...
              style={{ background: 'rgba(0,0,0,0.4)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: '4px', padding: '0.4rem', fontSize: '0.68rem', color: '#fff', fontFamily: 'var(--font-hud)' }}
            />

            <input 
              type="text" placeholder="Description (e.g. Main reduction gear temp critical.)" value={customDesc}
              onChange={(e) => setCustomDesc(e.target.value)}
              style={{ background: 'rgba(0,0,0,0.4)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: '4px', padding: '0.4rem', fontSize: '0.68rem', color: '#fff' }}
            />
            ...
            <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'center' }}>
              <span style={{ fontSize: '0.62rem', color: 'var(--text-secondary)' }}>Priority:</span>
              <select 
                value={customPriority} onChange={(e) => setCustomPriority(e.target.value)}
                style={{ background: '#020308', border: '1px solid rgba(255,255,255,0.08)', borderRadius: '4px', padding: '0.2rem', fontSize: '0.62rem', color: '#fff' }}
              >
              ...
              <button 
                ...
                style={{
                  ...
                  padding: '0.3rem 0.8rem', fontSize: '0.68rem', cursor: 'pointer', fontWeight: 'bold'
                }}
              >
                Inject Fault
              </button>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem', borderTop: '1px dashed rgba(255,255,255,0.08)', paddingTop: '0.6rem' }}>
            ...
            <div style={{ maxHeight: '110px', overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: '0.3rem' }}>
              {alerts.length === 0 ? (
                <span style={{ fontSize: '0.62rem', color: '#00e676', textAlign: 'center', display: 'block', padding: '0.5rem' }}>
                  ✓ Pilot dashboard has zero active alerts
                </span>
              ) : (
                alerts.map((al, idx) => (
      ...
      <footer style={{ textAlign: 'center', fontSize: '0.6rem', color: 'var(--text-secondary)', borderTop: '1px solid rgba(255,255,255,0.04)', paddingTop: '0.8rem' }}>
        COGNITIVESYNC HACKATHON COMPANION // OPEN THIS IN A SEPARATE TAB SIDE-BY-SIDE TO INJECT TACTICAL THREATS & EYE POSITION.
      </footer>
    </div>
  );
}
